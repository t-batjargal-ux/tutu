...
st.caption(
    "【プロトタイプ版: OpenAI駆動】列構造を完全に保護する、高精度・高安定性のデータクレンジング・プラットフォーム。"
)
st.markdown("---")
...
if uploaded_file is not None:
    ...
    with col2:
        st.markdown(
            "<p style='text-align: center; color: gray; margin-bottom: 5px;'>AIが「会社名」や「住所」の列を自動で特定し、列構造を壊さずに安全に整形します</p>",
            unsafe_allow_html=True,
        )
    ...
    if execute_button:
        with st.spinner("AI がデータの列構造を解析して精密クレンジングを実行中..."):
            try:
                # 作業用のレプリカを作成
                df_cleaned = df.copy()
                columns_list = list(df.columns)

                # --- 🤖 処理1: クレンジング対象の列名をAIに特定させる ---
                mapping_prompt = f"""
                以下の列名リストから、【会社名・取引先名】が格納されている列名と、【住所・所在地】が格納されている列名をそれぞれ1つずつ特定してください。
                列名リスト: {columns_list}

                必ず以下のJSON構造のみで返答してください。該当する列が一切ない場合は null にしてください。
                {{
                ...
                if company_col and company_col in df_cleaned.columns:
                    ...
                    comp_prompt = f"""
                    以下の文字列リスト（会社名）について、「㈱」や「(株)」などの略称をすべて「株式会社」に統一してください。
                    それ以外の文字は絶対に改変しないでください。入力と全く同じ要素数の配列を以下のJSON形式で返してください。
                    {{ "cleaned": ["値1", "値2", ...] }}
                    対象データ: {json.dumps(company_data, ensure_ascii=False)}
                    """
                    ...
                    cleaned_companies = json.loads(comp_res.choices[0].message.content).get("cleaned", [])
                ...
                if address_col and address_col in df_cleaned.columns:
                    ...
                    addr_prompt = f"""
                    以下の文字列リスト（住所）について、含まれる英数字、郵便番号、ハイフン、長音記号（ー）をすべて半角（ハイフンは「-」）に統一してください。
                    漢字の地名やビル名などは絶対に書き換えないでください。入力と全く同じ要素数の配列を以下のJSON形式で返してください。
                    {{ "cleaned": ["値1", "値2", ...] }}
                    対象データ: {json.dumps(address_data, ensure_ascii=False)}
                    """
                    ...
                    cleaned_addresses = json.loads(addr_res.choices[0].message.content).get("cleaned", [])
                    if len(cleaned_addresses) == len(address_data):
                        df_cleaned[address_col] = cleaned_addresses

                # セッション状態を更新し、コンポーネントのIDをリフレッシュ
                st.session_state.cleaned_df = df_cleaned
                st.session_state.refresh_counter += 1
                st.toast("✨ 列構造を維持したまま、精密クレンジングが完了しました！")
